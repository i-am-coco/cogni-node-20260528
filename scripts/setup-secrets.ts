...
const SECRETS: Secret[] = [
  ...
  },
  {
    ...
    url: "https://grafana.com/orgs",
    steps: [
      "Open https://grafana.com/orgs/<your-org-slug>/access-policies (Cloud portal — Security → Access policies in the left nav; NOT the stack admin UI). New to Grafana Cloud? Sign up at https://grafana.com/auth/sign-up/create-user first.",
      "Click 'Create access policy'. Display name: cogni-bootstrap-admin.",
      "Realm: 'all stacks' (org-wide) is fine for v1; or pick the specific stack you want bootstrap to be scoped to.",
      "**Default scope checkboxes are data-plane (metrics:write, logs:write, traces:write) — these are NOT what we need.** Scroll to the 'Add scope' dropdown near the bottom and add these FOUR admin scopes by name (one at a time): stacks:read, stack-service-accounts:write, accesspolicies:read, accesspolicies:write.",
      "Click 'Create'. On the new policy's detail page, click 'Add token' and copy the glc_ value (shown once).",
